        ...
        <div className="p-4 lg:p-8 max-w-7xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <Card className="border-l-4 border-l-orange-500">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-xs text-muted-foreground uppercase">{t("cryptoMetrics.btcDominance")}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-3xl font-bold">{formatPct(data.btc_dominance)}</p>
                    <p className="text-xs text-muted-foreground mt-1">{formatCap(data.btc_market_cap_usd)}</p>
                  </CardContent>
                </Card>
                <Card className="border-l-4 border-l-indigo-500">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-xs text-muted-foreground uppercase">{t("cryptoMetrics.altDominance")}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-3xl font-bold">{formatPct(data.alt_dominance)}</p>
                    <p className="text-xs text-muted-foreground mt-1">{formatCap(data.alt_market_cap_usd)}</p>
                  </CardContent>
                </Card>
                <Card className="border-l-4 border-l-emerald-500">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-xs text-muted-foreground uppercase">{t("cryptoMetrics.stableDominance")}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-3xl font-bold">{formatPct(data.stable_dominance)}</p>
                    <p className="text-xs text-muted-foreground mt-1">{formatCap(data.stable_market_cap_usd)}</p>
                  </CardContent>
                </Card>
              </div>
              ...
              <Card>
                <CardContent className="py-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">{t("cryptoMetrics.totalMarketCap")}</span>
                    <span className="text-xl font-bold">{formatCap(data.total_market_cap_usd)}</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className={cn("border-l-4", phaseColors[data.phase] || "border-l-slate-500")}>
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-sm font-medium">{t("cryptoMetrics.currentPhase")}</CardTitle>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="flex items-center gap-3 mb-2">
                    <span className={cn("px-2.5 py-1 rounded-md text-xs font-semibold", phaseBgColors[data.phase] || "bg-slate-500/10 text-slate-600")}>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-medium">{t("cryptoMetrics.capitalFlows")}</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card className="border-l-4 border-l-primary">
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-sm font-medium">{t("cryptoMetrics.signal")}</CardTitle>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="flex items-center gap-3 mb-2">
                    <span className={cn("px-3 py-1.5 rounded-md text-sm font-bold", signalColors[data.signal.type] || "bg-slate-500 text-white")}>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-sm font-medium">{t("cryptoMetrics.interpretation")}</CardTitle>
                  </div>
                </CardHeader>
                <CardContent>
                  <p className="text-sm text-muted-foreground leading-relaxed">
                ...
                <Card>
                  <CardHeader className="pb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-sm font-medium">{t("cryptoMetrics.historicalAnalog")}</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <p className="text-sm text-muted-foreground">{data.historical_match.period}</p>
