...
import { TrendingUp, TrendingDown, Activity, Wallet, AlertCircle, Loader2 } from "lucide-react"
...
interface FundamentalsData {
  ...
  composite: {
    ...
    sentiment: string
    components: {
      mvrv: { value: number; normalized: number; weight: number }
      nupl: { value: number; normalized: number; weight: number }
      funding: { value: number; normalized: number; weight: number }
    }
    interpretation: {
      mvrv: string
      nupl: string
  ...
  return (
    <Card className="bg-[#0b0f19] border-amber-500/30">
      ...
      <CardContent className="space-y-3">
            ...
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
              <div
                className={cn(
                  ...
                  data.nupl.value > 50 ? "bg-amber-500" :
        ...
        {data.composite && (
          <motion.div
            className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-800"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            <div className="text-center">
              ...
              <p className="text-[10px] font-mono font-bold text-slate-300">
                {data.composite.components.mvrv.normalized > 0 ? "+" : ""}
                {data.composite.components.mvrv.normalized.toFixed(2)}
              </p>
            </div>
            <div className="text-center">
              <p className="text-[9px] text-slate-500 uppercase">NUPL</p>
              <p className="text-[10px] font-mono font-bold text-slate-300">
                {data.composite.components.nupl.normalized > 0 ? "+" : ""}
                {data.composite.components.nupl.normalized.toFixed(2)}
              </p>
            </div>
            <div className="text-center">
              <p className="text-[9px] text-slate-500 uppercase">Funding</p>
              <p className="text-[10px] font-mono font-bold text-slate-300">
                {data.composite.components.funding.normalized > 0 ? "+" : ""}
                {data.composite.components.funding.normalized.toFixed(2)}
              </p>
            </div>
          </motion.div>
        )}
